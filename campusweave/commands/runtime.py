...
from ..private_artifacts import atomic_write_json, load_json_with_sha256, strict_load_json
from ..resources import CONCEPT_MANIFEST, REFERENCE_PROFILE, SOURCE_ROOT, TARGET_CONTEXT_TEMPLATE
from ..targets import validate_target_context
from .contracts import main as validate_machine_docs

REPOSITORY_ROOT = SOURCE_ROOT
DEFAULT_PROFILE = REFERENCE_PROFILE
DEFAULT_MANIFEST = CONCEPT_MANIFEST
DEFAULT_TARGET_TEMPLATE = TARGET_CONTEXT_TEMPLATE
...
def _profile(
    ...
    errors: list[str] = []
    concept_ids = profiles.concept_ids_from_manifest(manifest, errors)
    try:
    ...
    if not isinstance(document, Mapping):
        ...
        return None, None, sorted(set(errors))
    errors.extend(profiles.validate_package(document, concept_ids))
...
def _profile_instantiate(args: argparse.Namespace) -> int:
    ...
    concept_ids = profiles.concept_ids_from_manifest(args.manifest, concept_errors)
    concept_errors.extend(profiles.validate_package(document, concept_ids))
    if concept_errors:
...
def _target_validate(args: argparse.Namespace) -> int:
    ...
    if errors or profile is None or profile_digest is None:
        return _report_errors(args.profile, errors)
    try:
        checked_in_template = args.context.resolve(strict=True) == DEFAULT_TARGET_TEMPLATE.resolve(
            strict=True
        )
