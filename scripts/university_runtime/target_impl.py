...
def _validate_header(
    ...
    errors: list[str],
) -> tuple[Mapping[str, Any] | None, Mapping[str, Any] | None, Mapping[str, Any] | None, Mapping[str, Any] | None, Mapping[str, Any] | None, str | None, Any, str | None]:
...
def _validate_contract(contract: Mapping[str, Any] | None, loaded: dict[str, tuple[Any, str, Path, bytes]], errors: list[str]) -> tuple[Mapping[str, Any] | None, dict[str, Mapping[str, Any]]]:
    ...
    validated_operations = validate_machine_docs.validate_catalog(
        catalog_document, catalog_path, machine_errors
    )
    ...
    if isinstance(validated_operations, Mapping):
        for key, operation in validated_operations.items():
            if isinstance(key, str) and isinstance(operation, Mapping):
                ...
                errors.append("$.contract.catalog_path: catalog validator returned an invalid operation index")
                break
    else:
        errors.append("$.contract.catalog_path: catalog validator returned an invalid operation index")
    return operations
