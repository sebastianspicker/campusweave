...
def validate_policy_assignment_compatibility(
    ...
    if not isinstance(policy_models, list) or not isinstance(model, str) or model not in policy_models:
...
def _validate_unresolved_input(
    ...
    require_references(item.get("blocks_gate_ids"), gate_index, errors, f"{path}.blocks_gate_ids", nonempty=True)
