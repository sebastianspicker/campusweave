...
| `schemas/university-profile.schema.json` | Institution-neutral structural profile contract | JSON Schema 2020-12 with the stable identity `urn:campusweave-relution:schema:university-profile:1.0.0`; namespace, cross-document, and fail-closed rules are enforced by `campusweave.profiles` |
